...
GMP_STATIC_INLINE void ctl_dispatch(void)
{
    ...
    if (circ_cnt >= 200000)
    {
        ctrl_gt position = 4.0f * ctl_sin(2.0f * 3.14159f * (circ_cnt-200000)/250000);
        mech_ctrl.target_revs = (int32_t) position;
        mech_ctrl.target_angle = position - (ctrl_gt) ((int32_t)position);
    }
    circ_cnt++;
#endif
