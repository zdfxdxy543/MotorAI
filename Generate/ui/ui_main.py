...
class TuningResultPanel(QWidget):
    ...
    def _render(self, score_value, translated_summary, source_path_text, status_text=None):
    ...
    def _on_translation_success(self, summary_key: str, translated_text: str):
        ...
        self._render(self._current_score, self._translated_summary, self._last_source_path, '翻译完成')
    ...
    def _on_translation_failure(self, summary_key: str, error_text: str):
        ...
        self._render(self._current_score, self._translated_summary, self._last_source_path, f'翻译失败：{error_text}')
    ...
    def refresh_from_project(self):
        ...
        if not tuning_result_path or not tuning_result_path.exists():
            ...
            self._current_summary_text = ''
            self._last_source_path = ''
            self._render(None, '', '', '未找到 tuning_result.json')
            return
        ...
        try:
            ...
            self._render(None, f'读取失败：{exc}', str(tuning_result_path), f'读取 tuning_result.json 失败：{exc}')
            return

        final_evaluation = payload.get('final_evaluation') or {}
        score_value = final_evaluation.get('overall_score')
        summary_text = self._normalize_summary(payload.get('setup_summary'))
        summary_key = f'{tuning_result_path}:{summary_text}'
        ...
        if not summary_text:
            self._current_summary_key = ''
            self._translated_summary = ''
            self._render(score_value, '', str(tuning_result_path), 'setup_summary 为空')
            return

        if summary_changed:
            self._current_summary_key = summary_key
            self._translated_summary = ''
            self._render(score_value, '正在翻译 setup_summary...', str(tuning_result_path), '正在翻译 setup_summary')
            self._start_translation(summary_key, summary_text)
        else:
            self._render(score_value, self._translated_summary, str(tuning_result_path), 'overall_score 已刷新' if score_changed else None)
