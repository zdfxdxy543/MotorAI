...
class FrameworkDevGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GMP 核心框架资产管理器 V9 (架构分离与快捷键)")
        ...
        self.registry = self.model.registry # 映射内存引用，方便直接读取
    ...
    def mark_dirty(self):
        self.is_dirty = True
        self.root.title("GMP 核心框架资产管理器 V9 - *未保存的修改*")
    ...
    def apply_current_module(self, show_msg=False):
        ...
        # 【核心防呆拦截】：如果节点刚刚被删除，直接跳过保存，防止 KeyError！
    ...
    def clear_right_panel(self):
        ...
        self.entry_desc.delete(0, tk.END)
        for txt in (self.txt_src, self.txt_inc, self.txt_inc_dirs):
    ...
    def browse_path(self, target_widget, is_dir=False):
        ...
        chosen = filedialog.askdirectory(initialdir=initial_dir) if is_dir else filedialog.askopenfilename(initialdir=initial_dir)
        ...
        if is_dir and target_widget in (self.txt_src, self.txt_inc):
                ...
                if parent_key in self.registry["modules"][root_name]:
                    if messagebox.askyesno("转换", "是否将此模块转换为文件夹？"):
                        if self.current_selected_node == (root_name, parent_key):
                            self.current_selected_node = None # 焦点脱离，防止报错
