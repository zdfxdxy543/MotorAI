...
class FrameworkDevGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GMP Framework Asset Manager V16 (Markdown & DevOps)")
        ...
        # [NEW] Create markdown notes directory
    ...
    def mark_dirty(self):
        self.is_dirty = True
        self.root.title("GMP Framework Asset Manager V16 - *Unsaved Changes*")
    ...
    def build_ui(self):
        ...
        # ---------- Tab 3: Verification & DevOps Tracking (UPGRADED) ----------
        ...
        # [FIX 2.1] Buttons placed at the bottom, Listbox expands fully with dual scrollbars
        ...
        pw_tasks.add(task_edit_frame, weight=3) # Increased weight for larger editor
        ...
        # [FIX 2.3] Markdown Integration Buttons
        ...
        ttk.Button(lbl_reply_frame, text="📝 Open in Markdown Editor", command=self.open_task_markdown).pack(side=tk.RIGHT, padx=2)
        ttk.Button(lbl_reply_frame, text="🔄 Sync from MD", command=self.reload_task_markdown).pack(side=tk.RIGHT, padx=2)
        
        # [FIX 2.2] Expanding Text box with Dual Scrollbars
        ...
        # Make the text box expand fully
        task_edit_frame.rowconfigure(4, weight=1)
        task_edit_frame.columnconfigure(1, weight=1)
        
        ttk.Button(task_edit_frame, text="✔️ Save Task Changes", command=self.save_selected_task).grid(row=6, column=1, sticky=tk.E, pady=10)
    ...
    # ================= DevOps Tracking (Markdown Integrated) =================
    def get_current_task_md_path(self, task_id):
        """Generate safe markdown file path based on module and task id"""
    ...
    def on_task_select(self, event):
        ...
        # Auto-load from Markdown if exists, otherwise fallback to JSON reply
    ...
    def save_selected_task(self):
        ...
        # Save reply text to JSON AND auto-sync to Markdown file
    ...
    def open_task_markdown(self):
        """Dump current text to MD file and open it in the OS default editor"""
        selection = self.list_tasks.curselection()
        if not selection: return
        idx = selection[0]
        task = self.current_tasks_buffer[idx]
    ...
    def reload_task_markdown(self):
        """Reload text box from the MD file after user edited it externally"""
        selection = self.list_tasks.curselection()
        if not selection: return
        idx = selection[0]
        task = self.current_tasks_buffer[idx]
        ...
        if md_path and md_path.exists():
            try:
                ...
                # Auto-save buffer so the JSON catches the newly reloaded text
    ...
    def delete_custom_task(self):
        ...
        if messagebox.askyesno("Confirm Delete", f"Delete task '{task['summary']}'?"):
            # Clean up the associated MD file if it exists
            md_path = self.get_current_task_md_path(task.get("id"))
            if md_path and md_path.exists():
                try: md_path.unlink()
    ...
    def update_dep_folder_states(self, node=""):
        ...
        if node and (node.startswith("DEPFOLDER|") or node.startswith("DEPROOT|")):
            ...
            # [CRITICAL BUG FIX: Use self.tree_deps instead of self.tree]
