...
# 引入数据模型与新增加的诊断工具
from framework_data_model_v2 import FrameworkDataModel
import framework_validator
import framework_orphan_checker_v3  as framework_orphan_checker

class FrameworkDevGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GMP 核心框架资产管理器 V12 (引入诊断工具)")
    ...
    def mark_dirty(self):
        self.is_dirty = True
        self.root.title("GMP 核心框架资产管理器 V12 - *未保存的修改*")
        self.btn_global_save.config(state=tk.NORMAL)

    def mark_clean(self):
        self.is_dirty = False
        self.root.title("GMP 核心框架资产管理器 V12")
        self.btn_global_save.config(state=tk.DISABLED)

    def on_closing(self):
        if self.is_dirty:
            if messagebox.askyesno("退出提示", "有未保存修改，强行退出将丢失更改。\n确认退出吗？"):
                self.root.destroy()
        else:
            self.root.destroy()
    ...
    def build_ui(self):
        ...
        # 【新增】诊断工具栏
        tool_frame = ttk.LabelFrame(left_frame, text="🛠️ 诊断与校验工具", padding=5)
        tool_frame.pack(fill=tk.X, pady=5)
        ttk.Button(tool_frame, text="🔍 检查配置完整性 (找失效路径)", command=self.run_validation_tool).pack(fill=tk.X, pady=2)
        ttk.Button(tool_frame, text="👻 游离文件扫描 (找未包含文件)", command=self.run_orphan_tool).pack(fill=tk.X, pady=2)
        ...
        def create_scrolled_path_editor(row, label_text, is_help_doc=False):
            ...
            txt_widget = tk.Text(txt_frame, height=5, width=40, yscrollcommand=scroll.set)
            ...
            ttk.Button(lbl_frame, text="📌 插入宏", command=lambda event=None, w=txt_widget: self.popup_macro_menu(event, w)).pack(anchor=tk.W, fill=tk.X, pady=2)
            
            if is_help_doc:
                ttk.Button(lbl_frame, text="🌐 Windows打开文件", command=lambda: self.open_help_docs(txt_widget)).pack(anchor=tk.W, fill=tk.X, pady=2)
            
            return txt_widget

        self.txt_src = create_scrolled_path_editor(1, "源文件 (src_patterns):")
        self.txt_inc = create_scrolled_path_editor(2, "头文件 (inc_patterns):")
        self.txt_inc_dirs = create_scrolled_path_editor(3, "包含路径 (inc_dirs):")
        self.txt_help_docs = create_scrolled_path_editor(4, "帮助文档 (help_docs):\n(PDF/HTML/MD等)", is_help_doc=True)

        ttk.Label(self.tab_config, text="依赖模块树:\n(双击节点勾选)").grid(row=5, column=0, sticky=tk.NW, pady=5)
    ...
    def apply_current_module(self, show_msg=False):
        ...
        self.mark_dirty()
        if show_msg: messagebox.showinfo("暂存成功", f"模块 '{mod_key}' 的修改已暂存。")
    ...
    def open_help_docs(self, txt_widget):
        ...
        if not content:
            messagebox.showinfo("提示", "当前模块未配置帮助文档。")
            return
        ...
        if not abs_paths:
            messagebox.showwarning("警告", "未能解析到真实存在的文档文件！请检查路径。")
            return
    ...
    def run_validation_tool(self):
        ...
        self.display_report_window("完整性校验报告", report, is_error=has_error)

    def run_orphan_tool(self):
        if self.current_selected_node: self.apply_current_module(show_msg=False)
        # 默认忽略一些不希望被扫进来的文件夹
        ignores = ('.git', 'build', 'doc', 'docs', 'example', 'examples', 'suite', 'tools')
        has_orphans, report = framework_orphan_checker.run_orphan_check(self.model, ignore_dirs=ignores)
        self.display_report_window("游离资产扫描报告", report, is_error=has_orphans)
