...
class FrameworkDevGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GMP 核心框架资产管理器 V7 (全自动流与依赖树)")
    ...
    def mark_dirty(self):
        self.is_dirty = True
        self.root.title("GMP 核心框架资产管理器 V7 - *未保存的修改*")
    ...
    def build_ui(self):
        ...
        self.tree_deps.bind("<ButtonRelease-1>", self.on_dep_tree_click)
        # 标红无效依赖
    ...
    def refresh_dep_tree(self, depends_on_list, current_root, current_mod):
        ...
        missing_deps = set(depends_on_list) # 用于追踪失效的依赖
        ...
        for root_name in self.registry.get("roots", []):
            ...
            for mod_key in sorted_keys:
                if mod_key == f"{current_root}|{current_mod}": continue # 防自己依赖自己
                mod_data = modules[mod_key]
                if mod_data.get("type") == "module":
                    parts = mod_key.split('|')
                    parent_id = root_id
                    ...
                    if full_mod_path in missing_deps:
                        missing_deps.remove(full_mod_path) # 确认是有效依赖，从失踪名单剔除
    ...
    def apply_current_module(self, show_msg=False):
        """将右侧的修改只应用到内存中的 registry 字典 (无痕自动保存)"""
        ...
        target_mod["depends_on"] = self.get_all_checked_deps() # 从勾选树中提取
    ...
    def browse_path(self, target_widget, is_dir=False):
        ...
        # 【神级优化 3】：严格依据用户要求，如果是文件夹，则使用单层匹配 /*
        if is_dir:
