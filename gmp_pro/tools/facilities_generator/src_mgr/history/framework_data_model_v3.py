...
class FrameworkDataModel:
    ...
    def inject_default_tasks(self, mod_data):
        """Backward compatibility: Ensure tasks exist and contain mandatory system tasks"""
        if "tasks" not in mod_data:
            mod_data["tasks"] = copy.deepcopy(self.get_default_tasks())
        else:
            # Check if any system task is missing and inject it at the beginning
            existing_ids = [t.get("id") for t in mod_data["tasks"]]
            for dt in reversed(self.get_default_tasks()): # reversed to maintain order when inserting at 0
                if dt["id"] not in existing_ids:
                    mod_data["tasks"].insert(0, copy.deepcopy(dt))

    def get_module_status(self, mod_data):
        """
        Calculate module maturity level based on verification tasks.
        ...
        if task_map.get("sys_hw", False):
            ...
            return "🔷", "Sim/Principle Verified"  # Using Blue Diamond for triangle
    ...
    def load(self):
        ...
        # Auto-fill missing fields & tasks
        for root_name, modules in self.registry.get("modules", {}).items():
            for mod_data in modules.values():
                ...
                self.inject_default_tasks(mod_data)
        ...
        for root_name, modules in self.registry.get("modules", {}).items():
            ...
            for folder in folders:
                ...
                if internal_key not in modules:
                    ...
                    self.inject_default_tasks(modules[internal_key])
                    changed = True
        return changed
