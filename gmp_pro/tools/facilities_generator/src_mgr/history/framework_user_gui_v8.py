        ...
        try:
            ...
            if "[MODIFIED]" in output:
                ...
                confirm = messagebox.askyesno(
                    "危险动作确认", 
                    "已扫描出被修改的文件（详情请看背后控制台）。\n\n"
                    "您确定要将这些修改直接【物理覆盖】回 GMP 核心库吗？\n"
                    "警告：此操作不可逆！", 
                    parent=top, icon='warning'
                )
                
                if confirm:
                    # 第二阶段：执行真实的物理覆写 (附加 --apply 参数)
                    txt.insert(tk.END, "\n>>> 授权通过，正在拉起核心覆盖协议...\n")
                    top.update_idletasks()
