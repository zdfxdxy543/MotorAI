...
class FrameworkDevGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GMP 核心框架资产管理器 V11 (文档外链与超大视野)")
        self.root.geometry("1150x880") # 增加了窗口整体高度
    ...
    def mark_dirty(self):
        self.is_dirty = True
        self.root.title("GMP 核心框架资产管理器 V11 - *未保存的修改*")
    ...
    def open_help_docs(self, txt_widget):
        ...
        if success_count == 0:
            messagebox.showerror("错误", "调用系统程序打开文件失败，请确保电脑安装了对应的阅读器。")
    ...
    # ================= 实时匹配预览 (Live Preview) 完美修复版 =================
    def on_tab_changed(self, event):
        if self.notebook.index(self.notebook.select()) == 1:
            self.update_preview()

    def update_preview(self):
        """【修复】：抛弃 current_selected_node 检查，直接依据输入框的文字进行通配解析！"""
        for lb in (self.list_preview_src, self.list_preview_inc, self.list_preview_inc_dirs):
            lb.delete(0, tk.END)
            
        # 如果左侧树上啥也没选，直接返回
        if not self.tree.selection() or self.tree.selection()[0].startswith("ROOT|"):
            return
        
        # 无论当前是单个模块还是汇总的文件夹，此时文本框里都已经有文字了，直接读！
