...
class FrameworkUserGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GMP 工程框架配置器 V3 (全景预览与极简交互)")
    ...
    def build_ui(self):
        ...
        self.txt_det_src = create_detail_box(pw_detail, "📄 源文件规则 (src_patterns):")
        self.txt_det_inc = create_detail_box(pw_detail, "📁 头文件规则 (inc_patterns):")
        self.txt_det_dirs = create_detail_box(pw_detail, "⚙️ 包含目录依赖 (inc_dirs):")
        self.txt_det_deps = create_detail_box(pw_detail, "🔗 底层依赖关系 (depends_on):")
        self.txt_det_help = create_detail_box(pw_detail, "🌐 帮助文档 (help_docs):")
    ...
    def on_tree_double_click(self, event):
        ...
        # 【改进 3】：返回 "break" 阻止 Tkinter 默认的展开/折叠行为
        return "break"
    ...
    def on_tree_select(self, event):
        ...
        if node_id.startswith("ROOT|"):
            ...
            self.fill_detail_box(self.txt_det_help, mod_data.get("help_docs", []))
    ...
    def clear_detail_tab(self):
        for txt in (self.txt_det_src, self.txt_det_inc, self.txt_det_dirs, self.txt_det_deps, self.txt_det_help):
    ...
    def update_dashboard(self):
        ...
        if not self.selected_modules:
            self.notebook.tab(1, text="📊 配置汇总清单 (0)")
            self.notebook.tab(2, text="🗂️ 待同步文件 (C:0 | H:0)")
            return
        ...
        # 【改进 2】：动态分离标签数字与描述
        self.notebook.tab(1, text=f"📊 配置汇总清单 (模块:{len(self.selected_modules)})")
        self.notebook.tab(2, text=f"🗂️ 待同步文件 (C:{len(src_files)} | H:{len(inc_files)})")
