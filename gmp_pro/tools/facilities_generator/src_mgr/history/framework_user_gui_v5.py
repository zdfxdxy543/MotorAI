...
class FrameworkUserGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GMP 工程框架配置器 V5 (分离生成与反向同步接入)")
        self.root.geometry("1200x850")

        # 绑定 Ctrl+S 快捷键保存配置
        self.root.bind("<Control-s>", lambda e: self.save_local_config())
        self.root.bind("<Control-S>", lambda e: self.save_local_config())
    ...
    def build_ui(self):
        ...
        # 独立的保存按钮
        ttk.Button(fwd_frame, text="💾 仅保存配置 (Ctrl+S)", command=self.save_local_config).pack(side=tk.LEFT, padx=5)
        
        # 分离的生成按钮
        self.btn_sync_src = ttk.Button(fwd_frame, text="⬇️ 生成源文件 (Source)", command=lambda: self.save_and_sync("src_only"))
        self.btn_sync_src.pack(side=tk.LEFT, padx=5)
        
        self.btn_sync_inc = ttk.Button(fwd_frame, text="⬇️ 生成头文件树 (Header)", command=lambda: self.save_and_sync("inc_only"))
    ...
    def on_tree_select(self, event):
        ...
        # 3. 具体模块被点击 (因为是单个模块的配置，我们依然把它的物理路径解包给用户看)
    ...
    # ================= 保存与同步 =================
    def save_local_config(self, sync_mode="all", show_msg=True):
        config_data = {
            "sync_mode": sync_mode,
            "selected_modules": []
        }
    ...
    def save_and_sync(self, mode):
        if self.save_local_config(sync_mode=mode, show_msg=False):
            mode_text = {"src_only": "【生成源文件】", "inc_only": "【生成头文件树】"}
            messagebox.showinfo(
                "部署配置已就绪", 
                f"您请求了 {mode_text[mode]} 操作。\n\n"
                "部署意图已写入配置。\n请在工程目录下执行相应的 .bat 脚本，后台将根据此意图执行拉取！"
            )
            self.root.destroy()
    ...
    def reverse_sync_src(self):
        """【特性 2：反向同步安全保护】"""
        src_dir = Path(os.getcwd()) / "gmp_framework_src"
        if not src_dir.exists() or not any(src_dir.iterdir()):
            messagebox.showerror(
                "拦截：危险操作", 
                "本地不存在 'gmp_framework_src' 目录或该目录为空！\n\n"
                "由于您未生成源文件，执行反向同步可能会清空核心库的源码。\n请先执行【⬇️ 生成源文件】并完成修改后，再进行此操作。"
            )
            return
    ...
    def reverse_sync_inc(self):
        """【特性 2：反向同步安全保护】"""
        inc_dir = Path(os.getcwd()) / "gmp_includes"
        if not inc_dir.exists() or not any(inc_dir.iterdir()):
            messagebox.showerror(
                "拦截：危险操作", 
                "本地不存在 'gmp_includes' 目录或该目录为空！\n\n"
