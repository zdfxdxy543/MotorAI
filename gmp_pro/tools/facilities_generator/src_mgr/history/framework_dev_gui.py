...
class FrameworkDevGUI:
    def __init__(self, root):
        ...
        self.json_path = Path(self.gmp_location) / "tools" / "facilities_generator" / "framework_mgr" / "gmp_framework_dic.json"
        
        self.registry = {"roots": [], "modules": {}}
        self.current_selected_node = None 
        self.all_available_modules = [] # 缓存所有模块名，供依赖列表使用
    ...
    def on_tree_select(self, event):
        ...
        _, root_name, mod_name = node_id.split('|')
        ...
        for idx, available_mod in enumerate(self.all_available_modules):
            ...
            if available_mod in depends_on:
                self.listbox_deps.selection_set(tk.END) # 选中已有的依赖
