...
// Various styles of connect possible
// Note that there is only one option right now.  When additional options appear, we can choose if the
// options should be exclusive (only one value passed) or can be combined (or'ed together).  That will
// change what subsequent values of enum options should be (1,2,3,4 vs 1,2,4,8)
typedef enum GTI_CONNECT_OPTIONS_T
{
    CONNECT_OPT_LEGACY = 0,	// Default connect
    CONNECT_OPT_SACI = 1	// Connect in SACI mode, which is a explicit to loki devices
} GTI_CONNECT_OPTIONS;
...
/** GTI_CONNECT3() - Connect to target with specific connect flags to support different types of connections.
*
*
* \return 0 on success, -1 on error.
*/
...
GTI_CONNECT3(
    GTI_HANDLE_TYPE hpid, /**< [in]  GTI API instance handle. */
    GTI_CONNECT_OPTIONS flags /**< [in]  Connection flags */
);
